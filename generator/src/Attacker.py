...
import collections
import sys
import importlib.util  # substitui o módulo 'imp' (removido no Python 3.12+)
...
def doAttack(curAttack, msgOrig, a, attacksSuc):
    ...
    if RetVal['msg'] != None and RetVal['msg'] != 'Forward':
        RetVal['msg'].label = curAttack.__name__   # ground truth: nome do ataque
...
def loadAttacks(attacksToUse):
    ...
    for attack in attacksToUse:
        ...
        spec = importlib.util.spec_from_file_location(selectedModuleName, selectedModulePath)
