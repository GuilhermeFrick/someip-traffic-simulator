...
def writerWorker(q, serverCount, attackerList, attackerQueue, ServerDeviceConfig, ClientDeviceConfig, interface, pcap):
    ...
    packets = []   # acumula p/ escrita única (mais rápido que append por pacote)
    labels = []    # ground truth por pacote, alinhado ao PCAP
    ...
    while (True):
        ...
        if msg == 'Done':
            ...
            if pcap != None:
                ...
                labels.append(getattr(msg, 'label', 'normal'))
    ...
    # escrita única do PCAP + ground truth alinhado (índice do pacote -> rótulo)
    if pcap != None:
        wrpcap(pcap, packets)
        import numpy as _np
        _np.save(pcap + '.labels.npy', _np.array(labels))
        print ('Ground truth salvo em:', pcap + '.labels.npy')
